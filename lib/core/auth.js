...
export default class Auth {
    ...
    if (process.browser) {
      this.$storage.watchState('loggedIn', loggedIn => {
        if (!routeOption(this.ctx.route, 'auth', false)) {
          this.redirect(loggedIn ? 'home' : 'logout', false, this.strategy.name)
        }
      })
    }
  ...
  redirect (name, noRouter = false, strategyName = undefined) {
    ...
    if (strategyName) {
      to = __get(this.strategies[strategyName], 'options.redirect.' + name, false) || to
    }
    ...
    // Add prefix if exists and filter potential double slashes
    if (prefix) to = (prefix + to).replace(/([^:]\/)\/+/g, "$1")
